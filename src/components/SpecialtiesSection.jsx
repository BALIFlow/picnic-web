...
const IMGS = [
  { img: '/assets/margherita-close.jpg', alt: 'Pizza Margherita napolitana con mozzarella fresca y albahaca' },
  { img: '/assets/mortazza.jpg',          alt: 'Pizza Mortazza con mortadella, ricotta y pesto de pistacho' },
  { img: '/assets/stracciatella.jpg',     alt: 'Pizza Stracciatella con burrata, tomate cherry y pesto de albahaca' },
  { img: '/assets/pizza-oven.jpg',        alt: 'Pizza napolitana recién salida del horno de leña de Picnic' },
  { img: '/assets/carbonara.jpg',         alt: 'Espaguetis carbonara con guanciale crujiente y parmesano' },
  { img: '/assets/torta-pistacho.jpg',    alt: 'Torta de pistacho con crema de ricotta, postre de la casa' },
]

export default function SpecialtiesSection() {
  const { t } = useLang()
  const { label, title, titleItalic, subtitle, dishes } = t.specialties
    ...
    <section id="especialidades" className="bg-black py-24 md:py-32 px-5 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.06]">
          {dishes.map((d, i) => (
            <DishCard key={d.name} dish={{ ...d, ...IMGS[i] }} delay={i * 80} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <div ref={ref} className="reveal bg-[#0f0f0f] border border-white/[0.06] group relative overflow-hidden cursor-default"
      ...
      <div className="aspect-[4/3] overflow-hidden">
        <img src={dish.img} alt={dish.alt} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
      </div>
